...
object FStest {
  ...
  def main(args: Array[String]): Unit = {
	  val initStartTime = System.nanoTime()
		...
		val conf = new SparkConf().setAppName("FS test").setMaster("local[3]")
    ...
    val typeConversion = KeelParser.parseHeaderFile(sc, "/home/sramirez/datasets/ECBDL14.header") 
    val bcTypeConv = sc.broadcast(typeConversion)
    val lines = sc.textFile("/home/sramirez/datasets/ECBDL14/subSetROS.data")
    val data = lines.map(line => KeelParser.parseLabeledPoint(bcTypeConv.value, line))  
